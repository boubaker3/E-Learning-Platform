<template>
    <div class='w-1/2'>
      <h1 class="text-lg text-texts font-bold">Notifications</h1>
        ...
        <div v-for=" notif in notifications" class="items-center flex bg-inputs w-full p-2 rounded-md mt-4 text-texts ">
  <div class=" w-16 h-16 rounded  p-2">
    ...
    <div class='w-full flex items-center'>
          ...
          <div v-if="notif.type=='rating'" class="ml-2 flex items-center text-md">
        <p class="text-texts font-bold ">{{ notif.rating }} </p>
        <i class="fas fa-star text-primary-variant"></i> 
      </div>
      </div>
          <p class="text-sm font-bold text-primary">{{
      new Date(notif.created_at).toLocaleDateString('en-US',{year:"numeric",month:"long",day:'numeric'})
      }}</p>
...
import route from "../router";
  ...
  methods:{
    ...
    api.get(`showNotifications?userid=${user.userid}&page=${this.currentPage}` )
    .then(response=>{
