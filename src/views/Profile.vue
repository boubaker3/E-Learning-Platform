...
<div class="w-full  lg:w-1/2 h-auto shadow-2xl rounded-xl text-texts  p-4 mt-2">
...
<form @submit.prevent="follow">
    <button type="submit" v-if="userid!=this.$route.params.userid" class="text-texts hover:text-gray-800 px-4 py-2 bg-secondary ml-2 rounded p-2">
...
<h1 class="  bold text-texts text-sm md:text-base lg:text-lg xl:text-xl text-center mt-6">courses:</h1>
...
export default {
  ...
  fetchData(){
    ...
    api.get(`profileCourses?userid=${this.$route.params.userid}&page=${this.currentPage}`)
      .then(response => {
    ...
    follow() {
      api.post(`follow?receiverid=${this.$route.params.userid}&senderid=${this.userid}`)
        .then(response => { 
          this.checkFollowed();
        })
        .catch(error => {
    ...
    checkFollowed() {
      api.get(`followed?receiverid=${this.$route.params.userid}&senderid=${this.userid}`)
        .then(response => {
  ...
  computed: {
    photoPath() {
      return this.userdata.photo != "" ? localhost + "photos\\" + this.userdata.photo : userDefault
    }
  }, 
  watch:{
    '$route' (from,to){
