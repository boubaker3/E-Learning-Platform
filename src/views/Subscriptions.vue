<template>
  <h1 class="text-lg text-texts font-bold">Subscriptions</h1>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      api
        .get(`followingCourses?userid=${user.userid}&page=${this.currentPage}`)
        .then((response) => {
    ...
    incrementPage() {
      this.currentPage += 1;
      route.push(`/subscriptions/${this.currentPage}`);
    },
    decrementPage() {
      this.currentPage -= 1;
      route.push(`/subscriptions/${this.currentPage}`);
