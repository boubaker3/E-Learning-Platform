<template>
  <div class="w-full p-12">
    <div class="flex text-texts">
      <div
        @click="toProfile"
        class="w-12 h-12 rounded items-center flex justify-center p-2"
      >
        <img class="w-full rounded-md" :src="photoPath" />
      </div>
      <h1 @click="toProfile" class="mt-4 text-sm w-full">
        {{ userdata.fullname }}
      </h1>
      ...
      <form @submit.prevent="save">
        <button
          type="submit"
          class="h-10 text-texts hover:text-gray-800 px-4 py-2 bg-primary-variant ml-2 rounded"
        >
          Save
        </button>
      </form>
    ...
    <div class="flex w-full h-96 text-texts relative">
      ...
      <h1 class="text-base md:text-3xl bottom-0 m-4 absolute">
        {{ courseDetails.title }}
      </h1>
    </div>
    <div class="md:flex mt-6 text-texts font-bold">
      ...
      <div class="md:w-1/4 flex">
        <form @submit.prevent="addToCart">
          <div class="flex">
            <p class="text-3xl text-primary-variant">
              {{ courseDetails.price }}$
            </p>
            <button
              type="submit"
              class="text-texts hover:text-gray-800 px-4 py-2 bg-primary-variant ml-2 rounded"
            >
              Add to cart
            </button>
          </div>
    ...
    <div class="text-texts">
      <h1
        class="text-primary-variant font-bold text-base lg:text-lg xl:text-xl"
      >
        Course Details:
      </h1>

      <h2 class="font-bold text-base lg:text-lg xl:text-xl mt-6">
        What you're gonna learn:
      </h2>
      <div
        v-for="benefit in benefits"
        class="bg-inputs w-full md:w-1/2 p-2 rounded-md mt-4"
      >
        <p class="text-texts">{{ benefit }}</p>
      </div>

      <h2 class="text-base lg:text-lg xl:text-xl font-bold mt-6">
        Description:
      </h2>
      ...
      <div
        ...
        class="bg-primary-variant w-1/2 p-2 rounded-md mt-4"
      >
      ...
      <div class="mb-6">
        <form @submit.prevent="postReview">
          <div
            v-if="user.userid != courseDetails.userid"
            class="w-full md:flex mt-6"
          >
            <div class="flex w-full">
              <div
                class="w-16 h-16 rounded items-center flex justify-center p-2"
              >
              ...
              <input
                v-model="review"
                type="text"
                class="shadow-sm bg-inputs text-texts text-sm rounded-lg block w-full p-2.5"
          ...
          <button
            type="submit"
            class="text-texts hover:text-gray-800 px-4 py-2 bg-primary-variant ml-2 rounded mt-6"
          >
            Post
          </button>
        </form>
        ...
        >
          <div class="w-16 h-16 rounded p-2">
            <img
              ...
              :src="reviewPhotoPath(review.photo)"
            />
          </div>
          ...
          <div class="ml-6 flex items-center text-lg w-full">
            <i
              ...
              class="fas fa-star text-primary-variant flex"
            ></i>
          </div>
          <p class="text-sm">
            {{
...
export default {
  ...
  mounted() {
    ...
    api
      .get(
        `courseDetails?courseid=${this.$route.params.courseid}&userid=${this.$route.params.userid}`
      )
      .then((response) => {
        ...
        this.requirements = JSON.parse(
          response.data.courseDetails.requirements
        );
  ...
  methods: {
    ...
    getReviews() {
      api
        .get(`showReviews?courseid=${this.courseDetails.courseid}`)
        .then((response) => {
    ...
    reviewPhotoPath(photo) {
      return photo != null ? localhost + "photos\\" + photo : userDefault;
    },
  ...
  computed: {
    photoPath() {
      return this.userdata.photo != null
